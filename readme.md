...
scGET architecture is built using `Snakemake`: a workflow management system, which guarantees the possibility to **parallelize** independent jobs. **scGET** workflow is described by the image below: it starts from FASTQ files generated by the sequencing platform; it proceeds with the production of 2 BAM files, accounting for open chromatin regions (**tn5_bam**) and closed chromatin regions (**tnH_bam**); finally, it culminates with the PEAK COUNT of **tn5_h5ad** and **tnh_h5ad**, generating two `AnnData` objects, ready to be analyzed using `scany` library.
...
3. Similarly, also `samtools` must be installed:
   a. git repositories of `samtools` and `htslib` must be cloned
   b. `htslib` must be compiled and installed
   c. `samtools` must be compiled and installed
```
